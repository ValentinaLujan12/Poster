...
indices <- seq(from = 0, to = 1500, by = 30)

imagenes <- paste0("video_1/imagenes_bacteria", "/", "frame_", indices, ".png")
# Crear un dataset vacío
resultados_1 <- data.frame(Tiempo = numeric(),
                         Perimetro = numeric(),
                         Area = numeric())
...
for (i in seq_along(imagenes)) {
  ...
  tiempo_horas <- (i-1)*2.5 # Cada imagen representa 2.5 horas
  
  # Guardar los resultados en el dataset
  resultados_1 <- rbind(resultados_1, 
