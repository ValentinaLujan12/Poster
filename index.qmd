...
En la siguiente gráfica podrá observar el área obtenida de cada video y realizar una comparación más completa de los valores obtenidos en cada uno de ellos.
...
# Combinar los datos de los diferentes videos
datos_combinados <- bind_rows(
  resultados_1 %>% mutate(Video = "Video 1"),
  resultados_2 %>% mutate(Video = "Video 2"),
  resultados_3 %>% mutate(Video = "Video 3"),
  resultados_4 %>% mutate(Video = "Video 4")
)

# Aplicar la normalización a los datos de área
datos_combinados <- datos_combinados %>%
  group_by(Video) %>%
  mutate(Area_normalizada = normalizar(Area))

# Función para crear la gráfica con tonos de azul
crear_grafica_normalizada <- function(data) {  
  g <- ggplot(data, aes(x = Tiempo, y = Area_normalizada, color = Video)) +
    geom_line(linewidth = 1.5) +  
    labs(
      title = "Comparación de Áreas entre los 4 Videos",
      subtitle = "Crecimiento del microorganismo en función del área",
      x = "Tiempo (horas)",
      y = "Área Normalizada (escala 0 a 1)"
    ) +
    scale_color_manual(
      values = c("Video 1" = "#003f5c",  # Azul oscuro profundo
                 "Video 2" = "#0072B2",  # Azul mediano
                 "Video 3" = "lightblue",  # Azul-violeta suave
                 "Video 4" = "gray"), # Azul lavanda 
      name = "Video"
    ) +
...
# Graficar los datos normalizados con tonos de azul
crear_grafica_normalizada(datos_combinados)


```

Ahora, en la siguiente gráfica podrá observar la comparación del modelo logistico obtenido para cada video.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Filtrar solo los datos del modelo y combinar los datos de los videos
...
# Normalizar los datos de área predicha para cada video
datos_modelos_combinados <- datos_modelos_combinados %>%
  group_by(Video) %>%
  mutate(Area_normalizada = normalizar(Valor)) %>%
  ungroup()

# Función para crear la gráfica con los datos normalizados del modelo logístico
crear_grafica_modelo_normalizada <- function(data) {
  g <- ggplot(data, aes(x = Tiempo, y = Area_normalizada, color = Video)) +
    geom_line(linewidth = 1.5) +
    labs(
      title = "Comparación del Modelo Logístico entre los 4 Videos",
      subtitle = "Crecimiento del microorganismo (Área normalizada)",
      x = "Tiempo (horas)",
      y = "Área Normalizada (escala 0 a 1)"
    ) +
    scale_color_manual(
...
# Graficar los datos normalizados del modelo logístico
