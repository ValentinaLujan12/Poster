...
analizar_crecimiento_bacteria <- function(video_path, indice_sup, umbral_valor, otsu = FALSE, horas) {
  ...
  for (i in seq_along(imagenes)) {
    ...
    resultados <- rbind(resultados, 
                        data.frame(Imagen = imagenes[i], Tiempo = tiempo_horas, Perimetro = perimetro_milimetros, Area = area_milimetros2))
  }
  return(resultados)
}
...
crear_grafica <- function(datos) {  
  ...
  geom_line(aes(y = Area, color = "Área"), size = 1) +
  geom_line(aes(y = Perimetro, color = "Perímetro"), size = 1) +
     scale_color_manual(
      values = c("Perímetro" = "#0072B2", "Área" = "lightblue")) +
...
crear_grafica_modelo_normalizada <- function(data) {
  g <- ggplot(data, aes(x = Tiempo, y = Area_normalizada, color = Video)) +
    geom_line(linewidth = 1.2) +
    labs(
      title = "Comparación del Modelo Logístico",
      x = "Tiempo (horas)",
