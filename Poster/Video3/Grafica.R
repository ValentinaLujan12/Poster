...
ggplot(resultados_or, aes(x = Tiempo)) +
  # Líneas sólidas tanto para el perímetro como el área
  geom_line(aes(y = Perimetro, color = "Perímetro"), size = 1.5) +
  geom_line(aes(y = Area, color = "Área"), size = 1.5) +
  
  # Escalas adecuadas para el perímetro y el área
  scale_y_continuous(
    name = "Perímetro (milimetros)",
    sec.axis = sec_axis(~., name = "Área (milimetros cuadrados)")
  ) +
