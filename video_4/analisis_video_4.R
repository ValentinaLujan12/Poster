...
indices <- seq(from = 0, to = 2160, by = 30)

imagenes <- paste0("video_4/imagenes_bacteria", "/", "frame_", indices, ".png")
# Crear un dataset vacío
resultados_4 <- data.frame(Tiempo = numeric(),
                         Perimetro = numeric(),
                         Area = numeric())
...
for (i in seq_along(imagenes)) {
  ...
  tiempo_horas <- (i-1)*2.5 # Cada imagen representa 5 horas
  
  # Guardar los resultados en el dataset
  resultados_4 <- rbind(resultados_4, 
